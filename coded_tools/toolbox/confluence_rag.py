...
class ConfluenceRag(CodedTool, BaseRag):
    ...
    async def async_invoke(self, args: Dict[str, Any], sly_data: Dict[str, Any]) -> str:
        ...
        confluence_loader_params = [
            name
            for name in inspect.signature(ConfluenceLoader.__init__).parameters
            if name != "self"
        ]
        ...
        loader_args = {
            ...
            if arg in confluence_loader_params
        }
        ...
        # Resolve the backend connection (new args["vector_store"] + legacy shim;
        # Postgres creds prefer sly_data["vector_store"], fall back to env).
        vector_store_config = self.resolve_vector_store_config(args, sly_data)

        # Prepare the vector store through the shared backend factory.
    ...
    async def load_documents(self, loader_args: Dict[str, Any]) -> List[Document]:
        ...
        try:
            ...
            logger.error(
                "API Permission error while loading from %s: %s", url, api_error
            )
