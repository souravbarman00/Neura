...
"""
AWS DynamoDB automatic-memory store backend (P3-T5).

Automatic per-turn ``TopicStore`` surface over the shared
``coded_tools.toolbox.memory_backends`` DynamoDB adapter — one adapter
implementation, two surfaces (the explicit ``MemoryTool`` and this
``TopicStore``). The ``boto3`` client library is lazy-imported inside the
adapter's methods, so importing this module never requires ``boto3`` to
be installed.
...
class DynamoDBStore(TopicStore):
    """One DynamoDB item per ``(scoped-namespace, topic)``, scoped by user_id."""

    _BACKEND: ClassVar[str] = "dynamodb"
    ...
    def __init__(
        ...
        """
        Build the DynamoDB-backed store from the HOCON ``storage`` dict.
    ...
    def _build_conn(self, config: dict[str, Any] | None) -> dict[str, Any]:
        ...
        Connection secrets (access keys, endpoint, …) come ONLY from
