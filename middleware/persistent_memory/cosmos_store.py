...
"""
Azure Cosmos DB automatic-memory store backend (P3-T5).

Automatic per-turn ``TopicStore`` surface over the shared
``coded_tools.toolbox.memory_backends`` Cosmos adapter — one adapter
implementation, two surfaces (the explicit ``MemoryTool`` and this
``TopicStore``). The Azure client library is lazy-imported inside the
adapter's methods, so importing this module never requires
``azure-cosmos`` to be installed.
...
class CosmosStore(TopicStore):
    """One Cosmos item per ``(scoped-namespace, topic)``, scoped by user_id."""

    _BACKEND: ClassVar[str] = "cosmos"
    ...
    def __init__(
        ...
        """
        Build the Cosmos-backed store from the HOCON ``storage`` dict.
    ...
    def _build_conn(self, config: dict[str, Any] | None) -> dict[str, Any]:
        ...
        Connection secrets (endpoint, key, …) come ONLY from
